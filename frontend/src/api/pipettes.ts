...
import type { PipetteCreatePayload, PipetteListItem, PipetteDetail, CalibrationCreatePayload, Calibration } from "./types";
...
export async function createCalibration(pipetteId: string, payload: CalibrationCreatePayload): Promise<Calibration> {
  return apiRequest<Calibration>(`/api/pipettes/${pipetteId}/calibrations`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
}
