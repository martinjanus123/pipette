...
def _as_event_schema(event: PipetteEvent) -> PipetteEventSchema:
    return PipetteEventSchema(
        id=event.id,
...
def _as_detail(pipette: Pipette) -> PipetteDetail:
    detail = _as_list_item(pipette)
    # Load events if not already loaded
    events = sorted(pipette.events, key=lambda e: e.event_date, reverse=True)
    detail.events = [_as_event_schema(ev) for ev in events]
    return detail
...
@router.patch("/{pipette_id}/status", response_model=PipetteDetail)
def change_pipette_status(
    pipette_id: int, payload: PipetteStatusUpdate, db: DbSession
) -> PipetteDetail:
    pipette = db.scalar(
        select(Pipette)
        .where(Pipette.id == pipette_id)
    ...
    if old_status == new_status:
        # No change, but still return current detail
        return _as_detail(pipette)
